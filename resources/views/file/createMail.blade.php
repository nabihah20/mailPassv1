...
@section('content')
    ...
    {!! Form::open(['action' => 'FileController@sentMail','method' =>'POST','enctype' =>'multipart/form-data']) !!}
        <div class="form-group">
            ...
            {{Form::email('sender',auth()->user()->email,['readonly','class'=>'form-control','placeholder' => auth()->user()->email])}}
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            {{Form::text('title',$title = Session::get( 'title' ),['readonly','class'=>'form-control','placeholder' => $title = Session::get( 'title' )])}}
            {{Form::label('id','File Id')}}
            {{Form::text('id',$id = Session::get( 'id' ),['readonly','class'=>'form-control','placeholder' => $id = Session::get( 'id' )])}}
            {{Form::label('path','File Path')}}
            {{Form::text('path',$path = Session::get( 'path' ),['readonly','class'=>'form-control','placeholder' => $path = Session::get( 'path' )])}}
        </div>
